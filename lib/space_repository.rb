...
class SpaceRepository
  ...
  def create(space)
    sql = "INSERT 
              INTO spaces (title, description, address, price_per_night, 
                  available_from, available_to, host_id) 
              VALUES ($1, $2, $3, $4, $5, $6, $7);"
    DatabaseConnection.exec_params(sql, [
      space.title, space.description, 
      space.address, space.price_per_night, 
  ...
  def map_pg_to_objects(result_set)
    ...
    result_set.each do |record|
      space = assign_space(record)
      spaces << space
    end
    spaces
  end

  def assign_space(record)
    space = Space.new
